...
enum server_state {
    ...
    SERVER_STATE_TRANSITIONING,  // Model change in progress (load/unload/reload)
    SERVER_STATE_NO_MODEL,       // No model loaded, limited endpoints available
    ...
    void update() {
        ...
        for (const auto & state : states) {
            SRV_WRN("   - prompt %p: %7d tokens, checkpoints: %2zu, %9.3f MiB\n", (const void *) &state,
                    state.n_tokens(), state.checkpoints.size(), state.size() / (1024.0 * 1024.0));
        }
    }
};
...
struct server_slot {
    ...
    const common_chat_msg & update_chat_msg(std::vector<common_chat_msg_diff> & diffs) {
        ...
        auto new_msg = common_chat_parse(generated_text,
                                         /* is_partial= */ stop != STOP_TYPE_EOS, task->params.oaicompat_chat_syntax);
        if (!new_msg.empty()) {
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                    ...
                    if (slot.state == SLOT_STATE_STARTED) {
                        ...
                        if (!slot.can_split()) {
                            ...
                            if (slot.task->n_tokens() >= slot.n_ctx) {
                                send_error(slot, "the request exceeds the available context size, try increasing it",
                                           ERROR_TYPE_EXCEED_CONTEXT_SIZE);
                                slot.release();
                                continue;
                            }
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            for (auto & slot : slots) {
                ...
                for (size_t i = 0; i < draft.size(); ++i) {
                    common_batch_add(slot.batch_spec, draft[i], slot.prompt.tokens.pos_next() + 1 + i, { slot.id },
                                     true);
                }
                ...
                SLT_DBG(slot, "accepted %d/%d draft tokens, new n_tokens = %d\n", (int) ids.size() - 1,
                        (int) draft.size(), slot.prompt.n_tokens());
            }
        }
...
int main(int argc, char ** argv) {
    ...
    const auto handle_completions_impl = [&ctx_server, &res_error, &res_ok](
                                             server_task_type type, json & data, const std::vector<raw_buffer> & files,
                                             const std::function<bool()> & is_connection_closed,
        ...
        try {
            ...
            for (size_t i = 0; i < inputs.size(); i++) {
                ...
                if (n_prompt_tokens >= n_ctx_slot) {
                    json error_data =
                        format_error_response("the request exceeds the available context size, try increasing it",
                                              ERROR_TYPE_EXCEED_CONTEXT_SIZE);
    ...
    const auto handle_infill = [&ctx_server, &res_error, &handle_completions_impl](const httplib::Request & req,
        ...
        std::vector<raw_buffer> files;                 // dummy
        handle_completions_impl(SERVER_TASK_TYPE_INFILL, data, files, req.is_connection_closed, res,
    ...
    const auto handle_embeddings_impl = [&ctx_server, &res_error, &res_ok](const httplib::Request & req,
        ...
        if (!ctx_server.params_base.embedding) {
            res_error(res,
                      format_error_response("This server does not support embeddings. Start it with `--embeddings`",
                                            ERROR_TYPE_NOT_SUPPORTED));
            return;
        }
    ...
    const auto handle_rerank = [&ctx_server, &res_error, &res_ok](const httplib::Request & req,
        ...
        if (!ctx_server.params_base.embedding || ctx_server.params_base.pooling_type != LLAMA_POOLING_TYPE_RANK) {
            res_error(res, format_error_response("This server does not support reranking. Start it with `--reranking`",
                                                 ERROR_TYPE_NOT_SUPPORTED));
            return;
        }
        ...
        // write JSON response
        json root = format_response_rerank(body, responses, is_tei_format, documents, top_n);
    ...
    svr->Get(params.api_prefix + "/api/tags",
             handle_models);  // ollama specific endpoint. public endpoint (no API key check)
    ...
    LOG_INF("%s: chat template, chat_template: %s, example_format: '%s'\n", __func__,
            ...
            common_chat_format_example(ctx_server.chat_templates.get(), ctx_server.params_base.use_jinja,
